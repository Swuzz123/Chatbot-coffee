...
from retrieval.milvus_search import search_exact_item, general_search_and_recommned
...
def get_category_from_query(query):
    ...
    for main_cat in mappings.items():
...
def handle_user_query(query, limit=5):
    ...
    if is_exact:
        return search_exact_item()
    
    # Case 2: General recommend
    return general_search_and_recommned(query, limit)
